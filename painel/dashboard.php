...
if ($result && mysqli_num_rows($result) > 0) {
    $row = mysqli_fetch_assoc($result); // só associativo
    $usuario_nome = $row["nome"] ?? '';
    $usuario_email = $row["email"] ?? '';
    $usuario_celular = $row["celular"] ?? '';
    $usuario_cpf = $row["cpf"] ?? '';
    $usuario_permissao = $row["permissao"] ?? '';
    $celular_formatado = preg_replace('/(\d{2})(\d{1})(\d{4})(\d{4})/', '($1) $2 $3-$4', $usuario_celular);
    $foto64 = $row["foto_64"] ?? '';
} else {
...
<head>
    ...
    <!-- Ícones Feather (se precisar usar os mesmos do exemplo) -->
    <script src="https://unpkg.com/feather-icons"></script>

    <!-- Chart.js mais recente (para os gráficos do dashboard) -->
...
<body>
            ...
            <div class="sidebar border border-right col-md-3 col-lg-2 p-0 bg-body-tertiary">
                <div class="offcanvas-md offcanvas-end bg-body-tertiary" tabindex="-1" id="sidebarMenu" aria-labelledby="sidebarMenuLabel">
                    <div class="offcanvas-header">
                        <h5 class="offcanvas-title" id="sidebarMenuLabel">Clinica</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="offcanvas" data-bs-target="#sidebarMenu" aria-label="Close"></button>
                    </div>
                    <div class="offcanvas-body d-md-flex flex-column p-0 pt-lg-3 overflow-y-auto">
                        <ul class="nav flex-column">
                            <li class="nav-item">
                                <a id="menu-painel" class="nav-link d-flex align-items-center gap-2 active" aria-current="page" href="#">
                                    <svg class="bi" aria-hidden="true">
                        ...
                        <ul class="nav flex-column mb-auto">
                            <li class="nav-item">
                                <a class="nav-link d-flex align-items-center gap-2" href="./logoDEDM ,/ut.php">
                                    <svg class="bi" aria-hidden="true">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                    ...
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                        ...
                        <div class="btn-toolbar mb-2 mb-md-0">
                            <div class="btn-group me-2">
                                <button type="button" class="btn btn-sm btn-outline-secondary" id="btnBaixarPDF">
                                    Baixar PDF
                                </button>
                ...
                ";
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
                            <div class="card text-white bg-info shadow-sm border-0 rounded-4">
                                <div class="card-body d-flex justify-content-between align-items-center p-4">
                                    <div>
                                        <h3 class="fw-bold mb-1"><?php echo $row['total']; ?></h3>
                                        <span>Total de Agendamentos</span>
                                    </div>
                                    <i class="bi bi-table opacity-75" style="margin-right: 30px !important; margin-bottom: 50px !important; font-size: 40px;"></i>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Pendentes -->
                        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
                            <div class="card text-dark bg-warning shadow-sm border-0 rounded-4">
                                <div class="card-body d-flex justify-content-between align-items-center p-4">
                                    <div>
                                        <h3 class="fw-bold mb-1"><?php echo $row['pendentes']; ?></h3>
                                        <span>Pendentes</span>
                                    </div>
                                    <i class="bi bi-hourglass-split opacity-75" style="margin-right: 30px !important; margin-bottom: 50px !important; font-size: 40px;"></i>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
                            <div class="card text-white bg-danger shadow-sm border-0 rounded-4">
                                <div class="card-body d-flex justify-content-between align-items-center p-4">
                                    <div>
                                        <h3 class="fw-bold mb-1"><?php echo $row['cancelados']; ?></h3>
                                        <span>Cancelados</span>
                                    </div>
                                    <i class="bi bi-x-circle-fill opacity-75" style="margin-right: 30px !important; margin-bottom: 50px !important; font-size: 40px;"></i>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
                            <div class="card text-white bg-success shadow-sm border-0 rounded-4">
                                <div class="card-body d-flex justify-content-between align-items-center p-4">
                                    <div>
                                        <h3 class="fw-bold mb-1"><?php echo $row['concluidos']; ?></h3>
                                        <span>Concluídos</span>
                                    </div>
                                    <i class="bi bi-check-circle-fill opacity-75" style="margin-right: 30px !important; margin-bottom: 50px !important; font-size: 40px;"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <hr class="hr-1">
                    <h2>Tabela Geral de Clientes</h2>
                        ...
                        <table class="table table-striped ">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">Hora de Agenda.</th>
                            ...
                            <tbody>
                                ...
                                if (mysqli_num_rows($result) > 0) {
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        $linha_class = "";
                                        if ($status === "pendente") $linha_class = "table-warning";
                                        elseif ($status === "concluido") $linha_class = "table-success";
                                        elseif ($status === "cancelado") $linha_class = "table-danger";
                                ?>
                                        <tr class="<?= $linha_class ?>" style="font-size: 16px;">
                                            <th scope="row"><?= htmlspecialchars($row["id"]) ?></th>
                                            <td><?= htmlspecialchars($row["nome"]) ?></td>
                                            <td><?= htmlspecialchars($row["cpf"]) ?></td>
                                            <td><?= !empty($row["celular"]) ? preg_replace('/(\d{2})(\d{1})(\d{4})(\d{4})/', '($1) $2 $3-$4', $row["celular"]) : '' ?></td>
                                            <td><?= htmlspecialchars($row["email"]) ?></td>
                                            <td><?= !empty($row["data_agenda"]) ? date("d/m/Y", strtotime($row["data_agenda"])) : '' ?></td>
                                            <td><?= htmlspecialchars($row["hora_agenda"]) ?></td>
                                ...
                                <div class="card mb-3" style="border-radius: .5rem;">
                                    <div class="row g-0">
                                        <div class="col-md-4 gradient-custom text-center text-white"
                                            ...
                                            <?php
                                            if (!empty($foto64)) {
                                                $foto_exibir = "data:image/png;base64," . $foto64;
                                            } else {
                                            ...
                                            <h5 class="mt-3"><?= htmlspecialchars($usuario_nome) ?></h5>
                                        ...
                                        <div class="col-md-8">
                                            <div class="card-body p-4">
                                                ...
                                                <div class="row pt-1">
                                                    ...
                                                    <div class="col-6 mb-3">
                                                        <h6>Documento</h6>
                                                        <p class="text-muted"><?= htmlspecialchars($usuario_cpf) ?></p>
                                                    </div>
                                                </div>
                ...
                <div id="conteudo-permissoes" class="conteudo-oculto">
                    ...
                    <table class="table table-striped ">
                        ...
                        <tbody>
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $permissao = strtolower(trim($row["permissao"]));
                                    $linha_class = "";
                                    if ($permissao === "admin") $linha_class = "table-info";
                                    elseif ($permissao === "normal") $linha_class = "table-primary";                                    
                            ?>
                                    <tr class="<?= $linha_class ?>" style="font-size: 16px;">
                                        <th scope="row"><?= htmlspecialchars($row["id"]) ?></th>
                                        <td><?= htmlspecialchars($row["nome"]) ?></td>
                                        ...
                                        <td>
                                            <select class="permissao-select form-select form-select-sm" data-id="<?= $row['id'] ?>">
                                                <option value="admin" <?= $permissao === "admin" ? "selected" : "" ?>>Administrador</option>
                                                <option value="normal" <?= $permissao === "normal" ? "selected" : "" ?>>Normal</option>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            botoes.forEach(botao => {
                botao.addEventListener("click", function(e) {
                    ...
                    // Remove active e aplica no atual
                    ...
                    if (conteudo) {
                        ...
                        conteudo.classList.add("conteudo-ativo");
                    }
                });
            });
        });
        ...
        $(document).ready(function() {
            $('.permissao-select').change(function() {
                var permissao = $(this).val();
                var id = $(this).data('id');

                $.post('atualizar_permissao.php', {
                    id: id,
                    permissao: permissao
                }, function(response) {
                    if (response === 'success') {
